...
export default function ChangePasswordPage() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!PASSWORD_PATTERN.test(newPassword)) return setMessage("새 비밀번호는 영문, 숫자, 특수문자를 포함하여 12자리 이상이어야 합니다.");
    if (newPassword !== confirmPassword) return setMessage("새 비밀번호와 비밀번호 확인이 일치하지 않습니다.");
    if (currentPassword === newPassword) return setMessage("기존 비밀번호와 다른 비밀번호를 사용해 주세요.");
    ...
    try {
      ...
      const markResponse = await rest("rpc/mark_password_changed", verifiedSession.access_token, { method: "POST", body: JSON.stringify({}) });
      if (!markResponse.ok) throw new Error("비밀번호 변경 상태를 저장하지 못했습니다.");
      const profileResponse = await rest(`profiles?id=eq.${encodeURIComponent(verifiedSession.user.id)}&select=role,status`, verifiedSession.access_token);
      ...
      if (profile.role === "admin") router.replace("/admin/security");
  ...
  return (
    <AccountShell title="비밀번호 변경" description="기존 비밀번호를 확인한 후 새 비밀번호로 변경합니다. 관리자가 만든 계정은 최초 로그인 시 반드시 변경해야 합니다.">
      <form onSubmit={submit} className="grid gap-5">
        ...
        <label className="grid gap-2 text-sm font-semibold">새 비밀번호<input name="newPassword" required type="password" className={fieldClassName} autoComplete="new-password" placeholder="영문·숫자·특수문자 포함 12자리 이상" /></label>
