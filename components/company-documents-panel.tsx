...
export function CompanyDocumentsPanel({ companyId, showManageLink = false }: { companyId: string; showManageLink?: boolean }) {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        setMessage(error instanceof Error ? error.message : "회사 문서를 불러오지 못했습니다.");
  ...
  const download = async (document: CompanyDocument) => {
    const session = getSession();
    if (!session) { setMessage("로그인 후 문서를 내려받을 수 있습니다."); return; }
    try {
      setMessage("");
      const response = await storageRequest(
        `object/authenticated/company-documents/${document.storage_path.split("/").map(encodeURIComponent).join("/")}`,
        session.access_token,
      );
      if (!response.ok) throw new Error("문서를 내려받지 못했습니다. 접근권한 또는 파일 상태를 확인해 주세요.");
      ...
      setMessage(error instanceof Error ? error.message : "문서를 내려받지 못했습니다.");
    }
  };
  ...
  return (
    <section className="mt-8 overflow-hidden rounded-2xl border border-stone-300 bg-white shadow-sm">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-stone-200 px-5 py-4">
        <div>
          <div className="flex items-center gap-2 text-xs font-semibold tracking-[0.16em] text-emerald-800"><LockKeyhole className="h-4 w-4" /> COMPANY DOCUMENTS</div>
          <h2 className="mt-1 text-lg font-extrabold">회사 문서함</h2>
          <p className="mt-1 text-xs text-stone-500">정관·주주명부 등 회사별 비공개 보관 문서입니다.</p>
        </div>
        {showManageLink && (
          <Link href={`/admin/company/${companyId}/documents`} className="rounded-full border border-stone-300 px-4 py-2 text-xs font-bold text-stone-700 hover:bg-stone-50">문서 관리</Link>
        )}
      </div>
      ...
      {loading ? (
        ...
        <div className="px-5 py-7 text-sm text-stone-500"><p>등록된 회사 문서가 없습니다.</p><p className="mt-1 text-xs">정관·주주명부 등이 등록되면 이곳에서 내려받을 수 있습니다.</p></div>
      ) : (
        <div className="divide-y divide-stone-100">
          {documents.map((document) => (
            <div key={document.id} className="flex flex-wrap items-center justify-between gap-4 px-5 py-4">
              ...
              <button onClick={() => void download(document)} className="inline-flex items-center gap-2 rounded-full bg-emerald-900 px-4 py-2 text-xs font-bold text-white hover:bg-emerald-950"><Download className="h-4 w-4" /> 내려받기</button>
            </div>
          ))}
        </div>
      )}
