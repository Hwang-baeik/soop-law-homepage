...
export function CompanyRegistryViewer({ companyId }: { companyId: string }) {
  ...
  return (
    <section className="overflow-hidden rounded-2xl border border-stone-300 bg-white shadow-sm">
      <div className="flex flex-wrap items-center justify-between gap-4 border-b border-stone-200 px-5 py-4">
        ...
        <button onClick={loadLatest} type="button" className="inline-flex items-center gap-2 rounded-full border border-stone-300 px-4 py-2 text-xs font-bold text-stone-700 hover:bg-stone-50">
      ...
      <div className="border-b border-stone-200 bg-stone-50 p-5">
        ...
        <p className="mt-2 text-xs leading-5 text-stone-500">해당 회사의 소유주 또는 관리권한 회원이 PDF를 등록할 수 있습니다. 새 파일을 올리면 가장 최근 업로드본이 기본 표시됩니다.</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="p-10 text-center text-sm text-stone-500">등록된 등기부등본이 없습니다. PDF를 업로드하면 이 영역에서 바로 열립니다.</div>
      )}
