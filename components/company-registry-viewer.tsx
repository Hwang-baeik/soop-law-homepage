...
export function CompanyRegistryViewer({
  ...
  const loadLatest = async () => {
    if (!canViewDocuments) {
      setLoading(false); setDocument(null); setViewerUrl("");
      setMessage("이 계정에는 회사 문서 조회 권한이 없습니다.");
      return;
    }
    const session = getSession();
    if (!session) { setLoading(false); setMessage("로그인 후 등기부등본을 확인할 수 있습니다."); return; }
    ...
    try {
      ...
      setDocument(latest);
      if (!latest) { setViewerUrl(""); return; }

      const fileResponse = await storageRequest(
        `object/authenticated/company-documents/${storagePath(latest.storage_path)}`,
        session.access_token,
      );
  ...
  useEffect(() => { void loadLatest(); /* eslint-disable-next-line react-hooks/exhaustive-deps */ }, [companyId, canViewDocuments]);
  ...
  const upload = async () => {
    ...
    if (file.type !== "application/pdf" && !file.name.toLowerCase().endsWith(".pdf")) return setMessage("등기부등본은 PDF 파일만 업로드할 수 있습니다.");
    if (file.size > MAX_FILE_SIZE) return setMessage("파일 크기는 50MB 이하만 업로드할 수 있습니다.");

    const rawPath = `${companyId}/registry/${Date.now()}-${safeFileName(file.name)}`;
    try {
      setUploading(true); setMessage("");
      const uploadResponse = await storageRequest(`object/company-documents/${storagePath(rawPath)}`, session.access_token, {
        method: "POST",
      ...
      if (!uploadResponse.ok) {
        ...
        throw new Error(detail.message || "등기부등본 업로드에 실패했습니다.");
      }
      ...
      if (!metadataResponse.ok) {
        await storageRequest(`object/company-documents/${storagePath(rawPath)}`, session.access_token, { method: "DELETE" }).catch(() => null);
        const detail = await metadataResponse.json().catch(() => ({}));
        throw new Error(detail.message || "등기부등본 정보 저장에 실패했습니다.");
      }
  ...
  return (
    <section className="overflow-hidden rounded-2xl border border-stone-300 bg-white shadow-sm">
      <div className="flex flex-wrap items-center justify-between gap-4 border-b border-stone-200 px-5 py-4">
        ...
        {canViewDocuments && <button onClick={() => void loadLatest()} type="button" className="inline-flex items-center gap-2 rounded-full border border-stone-300 px-4 py-2 text-xs font-bold text-stone-700 hover:bg-stone-50"><RefreshCw className="h-4 w-4" /> 새로고침</button>}
      </div>

      {canUploadRegistry && (
        <div className="border-b border-stone-200 bg-stone-50 p-5">
          <div className="flex flex-col gap-3 md:flex-row md:items-center">
            <input type="file" accept="application/pdf,.pdf" onChange={(e) => setFile(e.target.files?.[0] || null)} className="min-w-0 flex-1 rounded-xl border border-dashed border-stone-300 bg-white p-3 text-sm" />
            <button type="button" onClick={() => void upload()} disabled={uploading} className="inline-flex h-11 items-center justify-center gap-2 rounded-full bg-emerald-900 px-5 text-sm font-bold text-white disabled:bg-stone-300"><FileUp className="h-4 w-4" /> {uploading ? "업로드 중..." : "등기부등본 업로드"}</button>
          </div>
          <p className="mt-2 text-xs leading-5 text-stone-500">PDF · 최대 50MB. 해당 회사에 업로드 권한이 있는 계정에만 표시됩니다.</p>
        </div>
      )}
      ...
      {loading ? (
        ...
        <div className="bg-stone-200 p-2 sm:p-4">
          <iframe title={document?.title || "법인 등기사항증명서"} src={`${viewerUrl}#page=1&view=FitH`} className="h-[72vh] min-h-[640px] w-full rounded-lg bg-white shadow-inner" />
          <div className="mt-2 flex flex-wrap items-center justify-between gap-2 text-xs text-stone-600"><span>{document?.file_name}</span><span>PDF 뷰어의 페이지 이동 버튼 또는 페이지 번호 입력으로 마지막 페이지까지 확인할 수 있습니다.</span></div>
        </div>
      ) : canViewDocuments ? (
