...
export async function verifyMfa(token: string, factorId: string, challengeId: string, code: string): Promise<AuthSession> {
  return authRequest(`factors/${encodeURIComponent(factorId)}/verify`, token, { method: "POST", body: JSON.stringify({ challenge_id: challengeId, code }) });
}
...
function emitAuthChanged() { if (typeof window !== "undefined") window.dispatchEvent(new Event("soop-auth-changed")); }
export function saveSession(session: AuthSession) { sessionStorage.setItem("soop_session", JSON.stringify(session)); emitAuthChanged(); }
export function getSession(): AuthSession | null {
  if (typeof window === "undefined") return null;
  const raw = sessionStorage.getItem("soop_session");
  if (!raw) return null;
  try { return JSON.parse(raw) as AuthSession; } catch { sessionStorage.removeItem("soop_session"); return null; }
}
export function clearSession() { sessionStorage.removeItem("soop_session"); emitAuthChanged(); }
